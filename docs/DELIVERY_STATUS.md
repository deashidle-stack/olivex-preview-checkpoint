...
- Reworked the Oleocanthal molecule side after follow-up feedback: removed decorative-only circles/line, moved points `01`-`04` into one left-side native details module under the molecule, placed `04 Kilde` at the bottom, removed visible `Kilde: Wikipedia`, kept Beauchamp/(som Ibuprofen)/bivirknings copy in the HTML, and verified Chrome viewports `1512 x 1100`, `1351 x 987`, `820 x 1180` and `390 x 900` with no overflow or summary-text overlap
